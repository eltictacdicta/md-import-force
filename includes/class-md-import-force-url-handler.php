    ...
    public static function detect_source_url($source_site_info, $posts_data) {
        try {
            ...
            if (!empty($source_site_info['site_url'])) {
                $url = rtrim($source_site_info['site_url'], '/') . '/';
                    ...
                    foreach ($post['images'] as $image) {
                        if (!empty($image['url'])) {
                            ...
                            if (!empty($url_parts['scheme']) && !empty($url_parts['host'])) {
                                ...
                                // Si hay un path, incluirlo completo para capturar subdirectorios de WordPress
                                if (!empty($url_parts['path'])) {
                                    // Obtener el path hasta el primer directorio de WordPress conocido
                                    $path = $url_parts['path'];

                                    // Buscar patrones comunes de WordPress y cortar el path en ese punto
                                    $wp_patterns = ['wp-content', 'wp-includes', 'wp-admin', 'wp-json'];
                                    ...
                                    // Si el path no está vacío, añadirlo a la URL base
                                    if (!empty($path)) {
                                        $base_url .= rtrim($path, '/');
                                    }
                                }
